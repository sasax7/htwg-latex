...
The system is designed as a modular Docker container that operates within the same \ac{AKS} cluster as the core Eliona microservices. This co-location ensures low-latency communication between the detection logic and the primary data storage. The architecture remains environment-agnostic, for on-premise deployments, the entire stack, including the analytics endpoint, can be executed locally as a suite of interconnected containers.
...
\begin{description}
	...
	\item[Analytics loop] After performing data preparation, the microservice transmits a request to the method endpoint.
