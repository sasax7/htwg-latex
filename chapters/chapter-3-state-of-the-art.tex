...
\section{Synthesis of Related Work}
\label{sec:synthesis-related-work}

The reviewed literature indicates a transition from deterministic expert systems and
regression baselines toward probabilistic deep learning architectures for time-series
anomaly detection. While rule-based and regression-based methods offer interpretability
and low computational overhead, they adapt poorly to multimodal and non-stationary
building operation. In parallel, the validity of reported progress is often undermined
by benchmark pathologies (e.g., mislabeling and run-to-failure bias) and by point-wise
metrics such as \ac{PA-F1}, motivating robust, threshold-independent evaluation via
\ac{VUS-PR}.

Deep generative models (e.g., OmniAnomaly) and compact neural architectures (e.g., CNNs
and variational autoencoders) frequently dominate benchmark leaderboards in
reconstruction- or residual-based formulations, and recent energy benchmarks show that
models trained from scratch can outperform \ac{TSFM} on labeled datasets such as
\ac{LEAD 1.0}. Conversely, \ac{TSFM} (e.g., Chronos-2) provide strong zero-shot
capabilities and native distributional forecasting, making them operationally attractive
for large portfolios.

Crucially, existing comparisons are largely confined to snapshot-style benchmarks and
deterministic residual paradigms that do not represent multivariate contextual
inefficiencies, regime-dependent multimodality, or long-term baseline drift. This leaves
open whether the reported dominance of compact models persists under realistic
non-stationary building regimes. Accordingly, this thesis evaluates probabilistic,
context-conditioned detection under controlled regime shifts and adopts a distributional
formulation that links detection to economic impact and hierarchical root-cause attribution.
