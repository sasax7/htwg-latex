...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.9\textwidth]{images/multivariate-timeseries.png}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.9\textwidth]{images/Energy-Causal-Chain.jpg}
...
\ac{HVAC} systems dominate building energy demand. Thermal gradients, solar radiation, humidity, and scheduling logic determine cooling and heating loads. Suboptimal control strategies, scheduling conflicts, and mechanical degradation induce baseline drift and excessive consumption, generating anomalies that are often operationally normal yet energetically inefficient.
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.7\textwidth]{images/mixture-destribution.png}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.9\textwidth]{images/Univariate-Multivariate-normalities.jpg}
	\caption{Schematic illustration of time series types along two axes---dimensionality (univariate vs.~multivariate) and normality regimes (single-mode vs.~multi-mode). Adapted from Boniol et al.'s tutorial on new trends in time series anomaly detection~\cite{boniol2023edbt-tutorial}.}
...
\subsection{Terminology: Multivariate and Multi-Target Time Series}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.8\textwidth]{images/anomaly_score.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{images/superviced-unsuperviced.jpg}
	\caption{Schematic overview of supervised, semi-supervised, and unsupervised learning paradigms for anomaly detection, adapted from Boniol et al.'s tutorial on new trends in time series anomaly detection~\cite{boniol2023edbt-tutorial}.}
...
\end{description}
In building-energy monitoring, the practical setting is predominantly semi-supervised:
historical telemetry is typically available and is treated as mostly nominal
baseline behavior, while explicit anomaly labels are sparse or absent.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{images/distance-dencity-forecast.jpg}
	\caption{Hierarchical taxonomy of anomaly detection methods grouped into distance-based, density-based, and prediction-based families, adapted from Boniol et al.'s tutorial on new trends in time series anomaly detection~\cite{boniol2023edbt-tutorial}.}
...
Each observation is assigned a binary label (normal vs. anomalous). Model predictions are evaluated using the confusion matrix, yielding counts of \ac{TP}, \ac{TN}, \ac{FP}, and \ac{FN}.
...
Building-energy telemetry is a multivariate, multimodal, and non-stationary stochastic
process governed by a causal chain spanning demand, control, and mechanical execution
(see Sections~\ref{subsec:building-energy-multivariate-ts}--\ref{subsec:causal-chain-energy-consumption}).
Consequently, deviations at aggregate meters typically reflect upstream technical or
behavioural faults rather than isolated numerical outliers.

From these structural and statistical properties, the following requirements for anomaly
detection follow (see Sections~\ref{subsec:building-energy-multivariate-ts},
\ref{subsec:temporal-autocorrelation-persistence}, and~\ref{subsec:communication-artifacts}):

\begin{enumerate}
	\item \textbf{Probabilistic multivariate modeling}\label{req:probabilistic-multivariate}\\
	Expected behaviour should be represented probabilistically to capture multimodal
	operating regimes and cross-variable dependencies (see
	Section~\ref{subsec:statistical-distribution-stochastic-noise}).

	\item \textbf{Robustness to temporal dependence and persistence}\label{req:temporal-robustness}\\
	Detection must remain stable under autocorrelation, seasonality, and long-duration
	persistence such that sustained deviations are not absorbed into the baseline (see
	Section~\ref{subsec:temporal-autocorrelation-persistence}).

	\item \textbf{Separation of physical faults and digital artifacts}\label{req:artifact-separation}\\
	Transmission gaps and aggregation artifacts must be separated from physical anomalies
	through explicit data-quality handling (see
	Section~\ref{subsec:communication-artifacts}).
\end{enumerate}

Building-energy telemetry is dominated by contextual point anomalies at aggregation
horizons relevant for monitoring (see
Sections~\ref{subsec:dimensionality-normality-modes} and~\ref{subsec:structural-classes-anomalies}).
Persistent deviations already present in historical baselines may be absorbed into learned
normality (normality drift), representing a fundamental constraint of semi-supervised and
